    ...
    <form method="POST" action="{{ route('admin.kelas.update', $kelas->id) }}">
        ...
        <div class="mb-6">
            ...
            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('nama_kelas') border-red-500 @enderror"
                   id="nama_kelas" name="nama_kelas" type="text" placeholder="Contoh: 8A" value="{{ old('nama_kelas', $kelas->nama_kelas) }}" required>
            @error('nama_kelas')
        ...
        <div class="mb-6">
            ...
            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('wali_kelas') border-red-500 @enderror"
                   id="wali_kelas" name="wali_kelas" type="text" placeholder="Contoh: Budi Santoso, S.Pd" value="{{ old('wali_kelas', $kelas->wali_kelas) }}" required>
            @error('wali_kelas')
                <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
            @enderror
        </div>
